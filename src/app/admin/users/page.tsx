...
function AdminUsersPageContent() {
  ...
  const resetPassword = async (userId: string, userName: string, userEmail: string) => {
    const ok = await confirm({
      ...
      message: `Будет сгенерирован временный пароль для «${userName}». Старый пароль перестанет работать. При входе пользователь обязан задать новый.`,
    ...
    try {
      ...
      if (!res.ok) {
        showToast(data.error || "Ошибка при сбросе пароля", "error")
        return
      }

      setCopiedTemp(false)
      setResetResult({ name: userName, email: userEmail, tempPassword: data.tempPassword })
    } catch {
                      ...
                      <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
                        ...
                        {isAdmin && (
                          <button
                            ...
                            className="p-2 text-gray-400 dark:text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950 rounded-lg transition-colors disabled:opacity-50"
                            title="Сбросить пароль"
                          >
                            {resettingId === user.id ? (
                              <RefreshCw className="h-4 w-4 animate-spin" />
      ...
      {resetResult && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-md mx-4 animate-in zoom-in-95 duration-200">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-2 text-sm text-gray-600 dark:text-slate-400">
                    Передайте его пользователю <strong>{resetResult.name}</strong> ({resetResult.email}).
                    При входе он обязан задать новый пароль. Пароль показывается только сейчас.
                  </p>
                </div>
              </div>

              <div className="mt-4 flex items-center gap-2">
                <code className="flex-1 px-3 py-2 rounded-lg bg-gray-100 dark:bg-slate-900 text-gray-900 dark:text-slate-100 font-mono text-sm break-all select-all">
                  {resetResult.tempPassword}
                </code>
                <button
                  onClick={copyTempPassword}
                  className="p-2 text-gray-500 dark:text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950 rounded-lg transition-colors shrink-0"
                  title="Скопировать"
                >
                  {copiedTemp ? (
            ...
            <div className="px-6 py-4 bg-gray-50 dark:bg-slate-900 rounded-b-xl flex justify-end">
