...
- Пиши ЖИВО и ЕСТЕСТВЕННО. Вопросы должны звучать как настоящий разговор с преподавателем, а не как тест.
...
- Варьируй тон и глубину: где-то спроси коротко и в лоб, где-то дай чуть больше контекста. Не делай все вопросы одинаковыми по структуре — живой преподаватель так не говорит.
- Тонкий, неочевидный вопрос ценнее прямолинейного. Вопрос, который заставляет задуматься и показать реальное понимание — вот цель.

ДЛИНА ВОПРОСА ЗАВИСИТ ОТ СЛОЖНОСТИ:
- easy: Максимум 1-2 коротких предложения. Прямой вопрос без вводных. Иногда — буквально одно предложение.
- medium: 1-2 предложения, допускается уточняющая мысль или условие. Не раздувай, но и не обрезай, если мысль требует пояснения.
- hard: 2-3 предложения. Вопрос должен раскрывать человека: составное условие, сценарий, связка аспектов. Hard-вопрос — это мини-кейс, который показывает, понимает ли студент суть, а не просто запомнил ответ. Каждое предложение несёт смысл, воды нет.
...
<examples>
ХОРОШИЙ easy-вопрос (короткий, в лоб):
"А что будет, если пользователь введёт пустую строку?"

ХОРОШИЙ easy-вопрос (живой, одно предложение):
"Зачем тут именно этот хук, а не обычная переменная?"

ХОРОШИЙ medium-вопрос (чуть больше контекста, уточняющая мысль):
"Ты вынес логику в отдельную функцию — а если бы нужно было поддержать ещё один формат данных, что бы пришлось менять?"

ХОРОШИЙ medium-вопрос (живой, с условием):
"Почему ты выбрал именно эту библиотеку? Что было бы, если данных в 10 раз больше?"

ХОРОШИЙ hard-вопрос (мини-кейс, раскрывает понимание):
"Допустим, нагрузка на API выросла в 50 раз и база стала узким местом. Что конкретно ты изменишь в архитектуре? Какие компромиссы это создаст?"

ХОРОШИЙ hard-вопрос (связка аспектов, заставляет рассуждать):
"Ты выбрал клиентский рендеринг. Как это повлияет на SEO и скорость первой загрузки? Если бы переводил на SSR — что придётся переделать и стоит ли оно того?"
...
→ Лучше (hard): "Что изменишь, если добавить ограничение Y? Какие части затронет сильнее всего и почему?"

ПЛОХОЙ вопрос (академический, мёртвый):
...
ПЛОХОЙ вопрос (шаблонный, одинаковый для всех):
"Какие преимущества и недостатки выбранного тобой подхода?"
→ Лучше (medium): "Ты использовал X вместо Y — в каком случае Y сработал бы лучше?"
...
6. Вопрос звучит живо и естественно? Если звучит шаблонно или как робот → ПЕРЕФРАЗИРУЙ.
7. Hard-вопрос раскрывает студента на 2-3 аспекта? Если нет → ДОРАБОТАЙ.
  ...
  "rejected_candidates": [
    {
      ...
      "reason_code": "DUPLICATE_SURFACE|DUPLICATE_SEMANTIC|TRIVIAL|ALREADY_ANSWERED|LOW_VALUE|TEMPLATE"
    }
  ],
  "coverage": {
...
- От 5 до 8 вопросов.
- Длина зависит от difficulty: easy — 1-2 предложения, medium — 1-2 предложения (чуть развёрнутее), hard — 2-3 предложения.
- Hard-вопросы ОБЯЗАНЫ раскрывать студента: составной сценарий, связка аспектов или мини-кейс, на который нельзя ответить заученной фразой.
- Вопросы должны быть ЖИВЫМИ и РАЗНООБРАЗНЫМИ — не штампуй одинаковую структуру. Где-то коротко и в лоб, где-то с контекстом. Живой преподаватель варьирует стиль.
