...
ПРИОРИТЕТ: при конфликте между разнообразием типов и точным соответствием исходному модулю — всегда выбирай точное соответствие модулю. Сохранение смысла и типа задачи важнее формального распределения question types.`
...
4. Ограниченное обогащение (bounded enrichment) допустимо и полезно, если модуль слишком бедный.
   ПОРОГ АКТИВАЦИИ: обогащение включается ТОЛЬКО если исходный материал реально неполон, слишком краткий или недостаточен для учебной завершённости. Если материал уже ясен и достаточно полон — передай его как есть, не расширяй без необходимости.
...
7. **Вопросы**: стремись к разнообразию типов, но при конфликте между разнообразием типов вопросов и точным соответствием исходному модулю — ВСЕГДА выбирай точное соответствие. Релевантность и сохранение типа задачи важнее распределения question types. Не создавай вопросы, уводящие модуль в другую деятельность.
