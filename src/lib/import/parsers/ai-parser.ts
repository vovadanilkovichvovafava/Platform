...
6. **Контент**: сохраняй в Markdown, но БЕЗ избыточности. Максимум 500 символов на поле content.
7. **Вопросы**: создавай разнообразные типы вопросов (не только SINGLE_CHOICE), но НЕ БОЛЕЕ 3 вопросов на модуль
8. **Улучшение**: если контент бедный - дополни кратко, без воды
...
11. **КРИТИЧНО - КОМПАКТНОСТЬ**: JSON должен быть максимально компактным! Избегай длинных описаний. Не создавай более 5 модулей на trail.`
...
export async function parseWithAI(
  ...
  try {
    ...
    // Используем максимальный лимит токенов для Claude (до 64k для sonnet/opus)
